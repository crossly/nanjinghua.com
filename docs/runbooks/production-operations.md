...
- 状态码 0：全部路径、轮次和运营商通过。
- 状态码 1：至少一个身份可信的探针已经确认站点失败；保留 JSON 报告中的测量 ID 和受控失败摘要，不得宣布正式上线。若同一报告另有基础设施错误，已确认的站点失败仍优先决定状态码。
- 状态码 2：没有已确认的站点失败，但发生参数错误、Globalping API 故障、缺少探针结果、探针身份不符或探针离线；不能据此判定站点通过或失败。
...
`.github/workflows/mainland-access.yml` 在每天 `00:17`、`08:17`、`16:17` UTC（中国标准时间 `08:17`、`16:17`、次日 `00:17`）只读执行默认三轮门禁，也支持人工触发。工作流固定官方 Action 的提交 SHA，不授予写权限，不部署 Worker、不运行迁移，也不读取 D1；可选仓库 secret `GLOBALPING_TOKEN` 只用于提高 Globalping API 配额。每次运行无论通过、站点失败还是基础设施错误都会上传 `mainland-access-<run id>-<attempt>` 工件，包含安全化 JSON、状态码和 stderr，保留 30 天；最终 job 状态仍按门禁状态码通过或失败。定时结果用于形成跨时间筛查序列，不替代真实终端，不允许用工件过期或单次绿色运行抹去已记录的失败窗口。

2026-07-20 起的[固定三网恢复复验](../releases/2026-07-20-mainland-recovery-recheck.md)首个窗口三轮 36/36，第二窗口 35/36；把 `www` 设为规范内容域后的第三窗口只有 24/36，上海移动对 `www` 的 12 次请求全部超时。恢复裸域后，`2026-07-22T02:55:37Z` 至 `02:56:17Z` 的第四窗口短暂取得 36/36；当前版本部署后的第五窗口于 `03:28:23Z` 至 `03:30:42Z` 再次降为 24/36，上海移动对裸域四条路径的 12 次请求全部超时；第六窗口于 `04:32:42Z` 至 `04:33:23Z` 又恢复到 36/36。原因未定的 Globalping 调用 `fetch failed`、当前无匹配探针、身份不符或离线均按基础设施错误与站点结果分开记录。成功、失败、再成功的交替结果仍未证明跨时间稳定性，真实终端体验门槛也未完成，继续保持非音频预览。
