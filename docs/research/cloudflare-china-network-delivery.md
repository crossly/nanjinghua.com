...
- `vite.config.ts:17-31` 已启用链接爬取和预渲染，排除 `/api/`、`/contribute`、`/browse` 及下载路径。构建产物已有首页、专题、档案和政策 HTML，说明公开只读内容具备静态交付基础。
- 当前 `assets.run_worker_first` 对除静态资源目录外的 `/*` 先运行 Worker；因此已有预渲染文件不等于现有生产路由已经是纯静态源站。
- `/browse` 当前被排除在预渲染之外，静态中国版本若要保留搜索，必须单独产出可水合的浏览页或明确降级；不能只复制现有静态文件后宣称四条验收路径都保留。
...
范围：主页、专题、档案详情、政策、静态图片、JS/CSS，以及补做后的 `/browse` 静态入口；不提供 `/contribute`、编辑 API、D1、Turnstile、定时保留任务或音频。
...
- 它不是现有配置的零改动部署，需要独立环境/配置把预渲染 HTML 交给 Assets，并使未知动态路径明确失败或跳转。
- `/browse` 需要产生静态可水合页面并在浏览器内搜索已发布内容；否则此前大陆验收的搜索路径会丢失。
- 所有需要 D1 或 Turnstile 的按钮和 API 必须在中国面诚实禁用，不能渲染一个必然失败的表单。
...
5. 先做只读静态 PoC，不连接生产 D1、不带生产 Secrets、不启用公众提交；确认 `nanjinghua.com` 的证书、IPv6、缓存、规范 URL、搜索和错误页。
