...
- 修复地址级诊断的固定探针复用后，于 `2026-07-22T03:58:39Z` 至 `03:59:02Z` 从同一上海移动探针复验：系统递归 DNS 正常返回两个 Cloudflare A 地址；`104.21.10.37` 的保留 Host/SNI HTTPS 为 200，`172.67.189.230` 的同等请求超时且 TCP MTR 未到达目标。四个测量 ID、旧版 selector 局限和安全化路径摘要见[固定三网恢复复验](./2026-07-20-mainland-recovery-recheck.md)。失败发生在 Worker 之前，不能通过 TanStack 路由或 D1 改动修复。
- 第六个三网窗口于 `2026-07-22T04:32:42Z` 至 `04:33:23Z` 恢复为 36/36：电信、联通、移动在三轮四路径中各 12/12。十二个测量 ID 与耗时范围见[固定三网恢复复验](./2026-07-20-mainland-recovery-recheck.md)。第四、第五、第六窗口依次成功、失败、成功，因此最新成功不能覆盖已确认的失败窗口，跨时间稳定性仍未成立。
...
- [固定三网恢复复验](./2026-07-20-mainland-recovery-recheck.md)已将移动回归点固定到上海：首个窗口 36/36，第二窗口 35/36；把 `www` 设为规范内容域后的第三窗口只有 24/36。恢复裸域后的第四窗口短暂取得 36/36，第五窗口再次降为 24/36，上海移动对裸域四条路径的 12 次请求全部超时，第六窗口又恢复到 36/36。交替结果仍未通过跨时间稳定性和真实终端验收。
