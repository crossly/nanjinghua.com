...
本次继续以 `https://nanjinghua.com` 作为全球非音频预览的规范地址目标，并将 `https://www.nanjinghua.com` 调整为到裸域的永久跳转；部署与线上复验完成前不把该调整表述为已生效。网站不称为正式首发或中国大陆正式上线。语音采集、真人音频、播放器、转写、R2 媒体及其直接依赖按产品决定停止后续实施与验收；没有使用合成音、静音或未授权音频替代。全球预览仍保留此前已交付的 `/recording-kit`、采集包下载和“录音意愿”线索类型，但当前不据此开展真人材料采集，也不把这些既有入口计入本轮完成范围。
...
- 0 个真人语音样本。
- 待部署源代码提交：将在规范域修复提交后填写。
- 待部署 Cloudflare Worker Version ID：将在唯一发布入口 `pnpm run deploy` 成功后填写。
...
- 此前发布的 `/`、`/browse?q=nanjing%20baiju`、`/api/archive/NJH000015`、`/api/submissions` 与 `/sitemap.xml` 均取得过 200 响应；搜索正文包含“南京白局”。规范域修复仍需部署后重新复验这些路径。
- Workers 预览域名此前保持 `noindex, nofollow`；规范域修复后还需重新核对正式域名与预览域名的响应头。
...
- 随后的[固定三网恢复复验](./2026-07-20-mainland-recovery-recheck.md)已将移动回归点固定到上海：首个窗口 36/36 通过，第二窗口为 35/36；把 `www` 设为规范内容域后的第三窗口只有 24/36，上海移动对 `www` 的 12 次请求全部超时。实现已据此恢复裸域为规范内容域，但该修复仍需部署后的同点复验；标准 Cloudflare 路径尚未通过跨时间稳定性和真实终端验收。
...
- `pnpm ops:validate:terminal -- <telecom|unicom|mobile>` 已把真实终端验收收敛为可审计命令：固定校验 ASN 和 CN 位置，执行三轮非音频页面、资源、规范 URL 和离线恢复，分别要求可见页面确认与目标运营商直连声明，并从报告中排除客户端 IP。当前工作连接的 headless 冒烟因出口身份不符且缺少两项声明被正确拒绝，不计入三网结果。
