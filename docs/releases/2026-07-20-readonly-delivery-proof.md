...
- 随后的[固定三网恢复复验](./2026-07-20-mainland-recovery-recheck.md)已将移动回归点固定到上海：首个窗口 36/36 通过，第二窗口为 35/36，唯一失败是上海移动访问非音频线索配置 API 超时。标准 Cloudflare 路径仍未通过跨时间稳定性和真实终端验收。
