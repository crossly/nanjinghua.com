...
test("公开命令不会让后续 Globalping API 故障覆盖已确认的站点失败", async () => {
	await withFakeGlobalping(
		{ failingHomeMobile: true, createStatus: 429, createFailurePath: "/browse" },
		async ({ baseUrl }) => {
			const result = await runValidator({
				GLOBALPING_API_BASE_URL: baseUrl,
				NANJINGHUA_MAINLAND_ROUNDS: "1",
			...
			assert.equal(report.outcome, "site-failure");
			const search = report.measurements.find((measurement) => measurement.route === "search");
			assert.ok(search);
			assert.equal(search.outcome, "infrastructure-error");
			assert.equal(search.id, null);
		},
	);
});
