...
type FakeGlobalpingOptions = {
	addresses: string[];
	createFailureType?: "dns" | "http" | "mtr";
	createFailureTarget?: string;
	...
	const server = createServer((request, response) => {
		void (async () => {
			...
			if (request.method === "POST" && url.pathname === "/v1/measurements") {
				...
				requests.push(body);
				if (
					body.type === options.createFailureType &&
					(options.createFailureTarget === undefined || body.target === options.createFailureTarget)
				) {
					respond(response, 429, { error: { message: "Too many requests via 203.0.113.53" } });
					return;
				}
...
test("大陆路由诊断不会让辅助 MTR API 故障覆盖已经确认的 HTTPS 失败", async () => {
	await withFakeGlobalping(
		{
			addresses: ["104.21.10.37"],
			failingAddress: "104.21.10.37",
			createFailureType: "mtr",
		},
		async (context) => {
			const result = await runDiagnostic("mobile", {
				GLOBALPING_API_BASE_URL: context.baseUrl,
			...
			assert.equal(result.code, 1);
			assert.doesNotMatch(result.stdout, /203\.0\.113\.53|Too many requests/);
			const report = JSON.parse(result.stdout) as {
				outcome: string;
				addresses: Array<{ mtr: { measurementId: string | null; infrastructureError: boolean } }>;
			};
			assert.equal(report.outcome, "site-failure");
			const [address] = report.addresses;
			assert.ok(address);
			assert.equal(address.mtr.measurementId, null);
			assert.equal(address.mtr.infrastructureError, true);
		},
	);
});
...
test("大陆路由诊断不会让另一地址的 API 故障覆盖已经确认的 HTTPS 失败", async () => {
	await withFakeGlobalping(
		{
			addresses: ["104.21.10.37", "104.21.10.38"],
			failingAddress: "104.21.10.37",
			createFailureType: "http",
			createFailureTarget: "104.21.10.38",
		},
		async (context) => {
			const result = await runDiagnostic("mobile", {
				GLOBALPING_API_BASE_URL: context.baseUrl,
			...
			const report = JSON.parse(result.stdout) as {
				outcome: string;
				addresses: Array<{
					https: { measurementId: string | null; infrastructureError: boolean };
				}>;
			};
			assert.equal(report.outcome, "site-failure");
			const secondAddress = report.addresses[1];
			assert.ok(secondAddress);
			assert.equal(secondAddress.https.measurementId, null);
			assert.equal(secondAddress.https.infrastructureError, true);
		},
	);
});

test("大陆路由诊断把 DNS API 故障归为受控的基础设施错误", async () => {
	await withFakeGlobalping({ addresses: [], createFailureType: "dns" }, async (context) => {
		const result = await runDiagnostic("mobile", {
			GLOBALPING_API_BASE_URL: context.baseUrl,
		});
		assert.equal(result.code, 2);
		...
		const report = JSON.parse(result.stdout) as {
			outcome: string;
			dns: { measurementId: string | null; infrastructureError: boolean };
		};
		assert.equal(report.outcome, "infrastructure-error");
		assert.equal(report.dns.measurementId, null);
		assert.equal(report.dns.infrastructureError, true);
	});
});
